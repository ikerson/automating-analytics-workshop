...
Session 4 confirmed the Oracle connection works and ran a single-table query. This session builds `db.py` v2: we replace the top-level exploration code with a five-table enrollment JOIN wrapped in a reusable function, and normalize the column names so the rest of the pipeline can use them consistently. The finished `db.py` is a module — it defines a function rather than running code at the top level — so it can be safely imported in later sessions without triggering a database query.
...
When you are finished exploring, remove the `if __name__ == '__main__':` block. The final `db.py` defines one constant and one function:
...
In Session 6, we shift to the API side of the data flow. `db.py` stays exactly as it is.
