...
Sessions 9 and 10 produced five DataFrames and two chart files. This session builds `report.py` v2: `save_excel_report()`, a function that writes a five-sheet Excel workbook and embeds the saved charts as images on a dedicated sheet. After this session, every output the pipeline is supposed to produce exists — and you can run all four modules in sequence by hand to verify the full result before wiring them together in Session 12.
...
> **This session does not require VPN or an API call.** The `__main__` block imports from `transform.py` and loads CSV files saved in prior sessions. It also requires the two chart `.png` files saved in Session 10. Make sure `top_middle_schools.png` and `school_size_distribution.png` exist in `student_report/reports/` before running.
...
After the last command, `student_report/reports/` should contain:
...
This is what the automated pipeline will produce with a single command in Session 12.
...
`report.py` is now complete. In Session 12, `main.py` wires all four modules together so that a single command produces every output file from scratch.
...
The complete working solution is at the bottom of `exercises/session_11_exercise.py`, commented out. Uncomment it to check your work.
